...
- (void)setItem:(OSCPhotoGroupItem *)item {
    ...
    @weakify(self);
    ///< 关闭使用缩略图作为打开大图的背景 (容错web端的缩略图裁剪问题)
    
    if (!item.largeImageURL) { //(容错web端的缩略图裁剪问题)
        [_imageView setImage:item.thumbImage];//(容错web端的缩略图裁剪问题)
    }else{//(容错web端的缩略图裁剪问题)
        [_imageView setImageWithURL:item.largeImageURL placeholder:nil options:YYWebImageOptionHandleCookies progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
- (instancetype)initWithGroupItems:(NSArray *)groupItems {
    ...
    _isUsingThumbViewDeedBG = NO;//(容错web端的缩略图裁剪问题)
...
//        _blurBackground.image = [_snapshorImageHideFromView imageByBlurDark]; //Same to UIBlurEffectStyleDark
    ...
    if (!cell.item) {
        cell.imageView.image = _isUsingThumbViewDeedBG ? item.thumbImage : nil;///< 取消动画启动图片浏览器 (容错web端的缩略图裁剪问题)
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        if (image) {
            ...
            [manger saveImage:image albumName:@"金陵贷" completeHandle:^(NSError *error, BOOL isHasAuthorized) {
