...
//    UIImageView *bgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"LXFBG"]];
//    bgView.userInteractionEnabled = YES;
//    bgView.contentMode = self.contentMode;
////    bgView.frame = CGRectMake(0, 0, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - 200);
//    [self addSubview:bgView];
//    [bgView mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.edges.equalTo(self);
//    }];
//        self.drawImageView = bgView;
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (self.drawStatus == LXFDrawStatusEnd) {
        [self.doManager addImage:self.curPaintImage];
//        self.curPaintImage = image;
    ...
    //    self.curPaintImage = [self snapsHotView:self.drawImageView];
...
//    UIGraphicsBeginImageContext(self.bounds.size);
    ...
    //    if ([self.brush isKindOfClass:[LXFMosaicBrush class]]) {
    //        self.style.lineColor = [self colorOfPoint:self.currentPoint];
    //        [[self colorOfPoint:self.currentPoint] setStroke];
