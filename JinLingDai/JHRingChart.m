...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _redius = (CGRectGetHeight(self.frame)-ChangedHeight(40))/3;
        _ringWidth = 20;
    }
    return self;
}
...
- (void)configBaseData{
    ...
    _totolCount = 0;
    _itemsSpace =  (M_PI * 2.0 * 1 / 360)/_valueDataArr.count ;
...
-(void)drawRect:(CGRect)rect{
    ...
    for (NSInteger i = 0; i<_valueDataArr.count; i++) {
        ...
        NSLog(@"begin - %@ endx -%@",NSStringFromCGPoint(begin),NSStringFromCGPoint(endx));
        ...
        if ([obj floatValue] / _totolCount * 100  > 1) {
